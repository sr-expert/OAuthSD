...
if ( CHECK_CLIENT_IP ) {
    ...
    if ( $code = @$_POST['code'] ) {       
        ...
        $stmt = $cnx->prepare($sql = "SELECT * FROM spip_authorization_codes WHERE authorization_code=:code");
    ...
    if ( !empty($client_id) ) {              
        // Get IP from clients
        $stmt = $cnx->prepare($sql = "SELECT * FROM spip_clients WHERE client_id=:client_id");
        ...
        if ( ! is_null($theip) ) {
            ...
            if ( strpos($theip, $remote) === false ) {     // client_ip may be multiple (LAN and WAN IPs, or client application sharing same registration)
                log_error("Token" , $message . ". Remote Adress = " . $remote . " not allowed for client = " . $client_id . ", waiting = " . $theip, $client_id, null, 300, 100, $cnx);
                if ( DEBUG ) {
                ...
                log_info("Token" , $message . " Ok. IP = " . $theip, $client_id, $user_id, 301, 0, $cnx);     
            }    
        }
        ...
        log_error("Token" , $message . " but client unknown", null, null, 304, 100, $cnx);
        if ( DEBUG ) {
