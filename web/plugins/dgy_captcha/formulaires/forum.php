...
function formulaires_forum_charger_dist($objet, $id_objet, $id_forum,
    ...
    if ($accepter_forum=="abo"){
        if ( empty($GLOBALS["visiteur_session"]['statut']) ){   //[dgy] empty() sinon plantage!
            return array(
...
function formulaires_forum_verifier_dist($objet, $id_objet, $id_forum,
    ...
    if (!count($erreurs) AND !_request('confirmer_previsu_forum')){
        if ($afficher_previsu!='non'){
            ...
            $erreurs['message_erreur'] = ''; //[dnc8] on ne veut pas du message_erreur automatique
        }
    }
    ...
    //[dgy6] Le formulaire de prévisualisation contient un Captcha
    if ( _request('confirmer_previsu_forum') ) {
        $captcha = _request('captcha2');  //[dgy6b] 
        session_start();  //[dgy6b] 
        //if ( empty($captcha) OR ($captcha!==session_get('captcha2')) ) {    //[dgy6b]
        if ( empty($captcha) OR ($captcha!==$_SESSION['captcha2']) ) {          //[dgy9]
            $erreurs['previsu'] = '<span style="color:red;"><b>' . _T('captcha2:erreur_captcha') . '</b></span><br /><br />';
        }
    }
