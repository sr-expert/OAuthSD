<?php

/**
 *  Fichier généré par la Fabrique de plugin v6
 *   le 2016-10-08 07:24:51
 *
 *  Ce fichier de sauvegarde peut servir à recréer
 *  votre plugin avec le plugin «Fabrique» qui a servi à le créer.
 *
 *  Bien évidemment, les modifications apportées ultérieurement
 *  par vos soins dans le code de ce plugin généré
 *  NE SERONT PAS connues du plugin «Fabrique» et ne pourront pas
 *  être recréées par lui !
 *
 *  La «Fabrique» ne pourra que régénerer le code de base du plugin
 *  avec les informations dont il dispose.
 *
**/

if (!defined("_ECRIRE_INC_VERSION")) return;

$data = array (
  'fabrique' => 
  array (
    'version' => 6,
  ),
  'paquet' => 
  array (
    'prefixe' => 'oauth',
    'nom' => 'OAuth 2.0',
    'slogan' => 'Helps building an OAuth 2.0 Server',
    'description' => '',
    'logo' => 
    array (
      0 => '',
    ),
    'version' => '1.0.0',
    'auteur' => 'DnC',
    'auteur_lien' => 'http://degoy.com',
    'licence' => 'GNU/GPL',
    'categorie' => 'outil',
    'etat' => 'dev',
    'compatibilite' => '[3.1.3;3.1.*]',
    'documentation' => 'http://oa.dnc.global',
    'administrations' => 'on',
    'schema' => '1.0.0',
    'formulaire_config' => '',
    'formulaire_config_titre' => '',
    'fichiers' => 
    array (
      0 => 'autorisations',
      1 => 'fonctions',
      2 => 'options',
      3 => 'pipelines',
    ),
    'inserer' => 
    array (
      'paquet' => '',
      'administrations' => 
      array (
        'maj' => '',
        'desinstallation' => '',
        'fin' => '',
      ),
      'base' => 
      array (
        'tables' => 
        array (
          'fin' => '',
        ),
      ),
    ),
    'scripts' => 
    array (
      'pre_copie' => '',
      'post_creation' => '',
    ),
    'exemples' => '',
  ),
  'objets' => 
  array (
    0 => 
    array (
      'nom' => 'users',
      'nom_singulier' => 'user',
      'genre' => 'masculin',
      'logo' => 
      array (
        0 => '',
        32 => '',
        24 => '',
        16 => '',
        12 => '',
      ),
      'logo_variantes' => '',
      'table' => 'spip_oa_users',
      'cle_primaire' => 'id_oauth_user',
      'cle_primaire_sql' => 'bigint(21) NOT NULL',
      'table_type' => 'oauth_user',
      'champs' => 
      array (
        0 => 
        array (
          'nom' => 'Pseudo',
          'champ' => 'username',
          'sql' => 'varchar(255) NOT NULL DEFAULT \'\'',
          'caracteristiques' => 
          array (
            0 => 'editable',
            1 => 'obligatoire',
          ),
          'recherche' => '',
          'saisie' => 'input',
          'explication' => 'Entrez un pseudonyme',
          'saisie_options' => '',
        ),
        1 => 
        array (
          'nom' => 'Mot de passe',
          'champ' => 'password',
          'sql' => 'varchar(2000)',
          'caracteristiques' => 
          array (
            0 => 'editable',
            1 => 'obligatoire',
          ),
          'recherche' => '',
          'saisie' => 'input',
          'explication' => 'Saisissez un mot de passe',
          'saisie_options' => '',
        ),
        2 => 
        array (
          'nom' => 'Prénom',
          'champ' => 'first_name',
          'sql' => 'varchar(255)',
          'caracteristiques' => 
          array (
            0 => 'editable',
            1 => 'obligatoire',
          ),
          'recherche' => '',
          'saisie' => 'input',
          'explication' => 'Votre prénom',
          'saisie_options' => '',
        ),
        3 => 
        array (
          'nom' => 'Nom',
          'champ' => 'last_name',
          'sql' => 'varchar(255)',
          'caracteristiques' => 
          array (
            0 => 'editable',
            1 => 'obligatoire',
          ),
          'recherche' => '',
          'saisie' => 'input',
          'explication' => 'Votre nom',
          'saisie_options' => '',
        ),
        4 => 
        array (
          'nom' => 'E-mail',
          'champ' => 'email',
          'sql' => 'varchar(256)',
          'caracteristiques' => 
          array (
            0 => 'editable',
            1 => 'obligatoire',
          ),
          'recherche' => '',
          'saisie' => 'input',
          'explication' => 'Entrez un E-mail valide et permanent. Cet E-mail servira aux échanges techniques et ne sera pas divulgué.',
          'saisie_options' => '',
        ),
      ),
      'champ_titre' => '',
      'champ_date' => 'date_publication',
      'statut' => 'on',
      'chaines' => 
      array (
        'titre_objets' => 'Users',
        'titre_objet' => 'User',
        'info_aucun_objet' => 'Aucun user',
        'info_1_objet' => 'Un user',
        'info_nb_objets' => '@nb@ users',
        'icone_creer_objet' => 'Créer un user',
        'icone_modifier_objet' => 'Modifier ce user',
        'titre_logo_objet' => 'Logo de ce user',
        'titre_langue_objet' => 'Langue de ce user',
        'texte_definir_comme_traduction_objet' => 'Ce user est une traduction du user numéro :',
        'titre_objets_rubrique' => 'Users de la rubrique',
        'info_objets_auteur' => 'Les users de cet auteur',
        'retirer_lien_objet' => 'Retirer ce user',
        'retirer_tous_liens_objets' => 'Retirer tous les users',
        'ajouter_lien_objet' => 'Ajouter ce user',
        'texte_ajouter_objet' => 'Ajouter un user',
        'texte_creer_associer_objet' => 'Créer et associer un user',
        'texte_changer_statut_objet' => 'Ce user est :',
        'supprimer_objet' => 'Supprimer cet user',
        'confirmer_supprimer_objet' => 'Confirmez-vous la suppression de cet user ?',
      ),
      'table_liens' => '',
      'roles' => '',
      'auteurs_liens' => '',
      'vue_auteurs_liens' => '',
      'autorisations' => 
      array (
        'objet_creer' => 'administrateur',
        'objet_voir' => 'redacteur',
        'objet_modifier' => 'administrateur',
        'objet_supprimer' => 'administrateur',
        'associerobjet' => 'administrateur',
      ),
    ),
    1 => 
    array (
      'nom' => 'Clients',
      'nom_singulier' => 'client',
      'genre' => 'masculin',
      'logo' => 
      array (
        0 => '',
        32 => '',
        24 => '',
        16 => '',
        12 => '',
      ),
      'logo_variantes' => '',
      'table' => 'spip_oa_clients',
      'cle_primaire' => 'client_id',
      'cle_primaire_sql' => 'bigint(21) NOT NULL',
      'table_type' => 'client_id',
      'champs' => 
      array (
        0 => 
        array (
          'nom' => 'Client secret',
          'champ' => 'client_secret',
          'sql' => 'varchar(80) DEFAULT NULL',
          'caracteristiques' => 
          array (
            0 => 'editable',
            1 => 'obligatoire',
          ),
          'recherche' => '',
          'saisie' => 'input',
          'explication' => 'Entrez une phrase secrète',
          'saisie_options' => '',
        ),
        1 => 
        array (
          'nom' => 'Redirect uri',
          'champ' => 'redirect_uri',
          'sql' => 'varchar(2000) NOT NULL',
          'caracteristiques' => 
          array (
            0 => 'editable',
            1 => 'obligatoire',
          ),
          'recherche' => '',
          'saisie' => 'input',
          'explication' => 'URI de retour à l\'application cliente (http://domain.ext/application etc.)',
          'saisie_options' => '',
        ),
        2 => 
        array (
          'nom' => 'Grant types',
          'champ' => 'grant_types',
          'sql' => 'varchar(80) DEFAULT NULL',
          'caracteristiques' => 
          array (
            0 => 'editable',
            1 => 'obligatoire',
          ),
          'recherche' => '',
          'saisie' => 'selection',
          'explication' => 'Sélectionnez la méthode d\'authentification (Grant Type)',
          'saisie_options' => 'datas=[(#ARRAY{
AuthorizationCode,Authorization Code,
Implicit,Implicit,
UserCredentials,User Credentials,
ClientCredentials,Client Credentials (server-server),
RefreshToken,Refresh Token,
JwtBearer,Jwt Bearer
})]',
        ),
        3 => 
        array (
          'nom' => 'Scope',
          'champ' => 'scope',
          'sql' => 'varchar(100) DEFAULT NULL',
          'caracteristiques' => 
          array (
            0 => 'editable',
            1 => 'obligatoire',
          ),
          'recherche' => '',
          'saisie' => 'textarea',
          'explication' => 'Une liste de Types d\'Autorisation, séparés par des points-virgules.',
          'saisie_options' => 'rows=4',
        ),
        4 => 
        array (
          'nom' => 'User id',
          'champ' => 'user_id',
          'sql' => 'varchar(80) DEFAULT NULL',
          'recherche' => '',
          'saisie' => '',
          'explication' => '',
          'saisie_options' => '',
        ),
      ),
      'champ_titre' => '',
      'champ_date' => '',
      'statut' => 'on',
      'chaines' => 
      array (
        'titre_objets' => 'Oauth clients',
        'titre_objet' => 'Oauth client',
        'info_aucun_objet' => 'Aucun oauth client',
        'info_1_objet' => 'Un oauth client',
        'info_nb_objets' => '@nb@ oauth clients',
        'icone_creer_objet' => 'Créer un oauth client',
        'icone_modifier_objet' => 'Modifier ce oauth client',
        'titre_logo_objet' => 'Logo de ce oauth client',
        'titre_langue_objet' => 'Langue de ce oauth client',
        'texte_definir_comme_traduction_objet' => 'Ce oauth client est une traduction du oauth client numéro :',
        'titre_objets_rubrique' => 'Oauth clients de la rubrique',
        'info_objets_auteur' => 'Les oauth clients de cet auteur',
        'retirer_lien_objet' => 'Retirer ce oauth client',
        'retirer_tous_liens_objets' => 'Retirer tous les oauth clients',
        'ajouter_lien_objet' => 'Ajouter ce oauth client',
        'texte_ajouter_objet' => 'Ajouter un oauth client',
        'texte_creer_associer_objet' => 'Créer et associer un oauth client',
        'texte_changer_statut_objet' => 'Ce oauth client est :',
        'supprimer_objet' => 'Supprimer cet oauth client',
        'confirmer_supprimer_objet' => 'Confirmez-vous la suppression de cet oauth client ?',
      ),
      'table_liens' => 'on',
      'vue_liens' => 
      array (
        0 => 'spip_auteurs',
      ),
      'roles' => '',
      'auteurs_liens' => '',
      'vue_auteurs_liens' => '',
      'fichiers' => 
      array (
        'explicites' => 
        array (
          0 => 'action/supprimer_objet.php',
        ),
      ),
      'saisies' => 
      array (
        0 => 'objets',
      ),
      'autorisations' => 
      array (
        'objet_creer' => '',
        'objet_voir' => 'redacteur',
        'objet_modifier' => '',
        'objet_supprimer' => '',
        'associerobjet' => '',
      ),
    ),
  ),
  'images' => 
  array (
    'paquet' => 
    array (
      'logo' => 
      array (
        0 => 
        array (
          'extension' => 'gif',
          'contenu' => '',
        ),
      ),
    ),
    'objets' => 
    array (
      0 => 
      array (
      ),
      1 => 
      array (
      ),
    ),
  ),
);