...
function oidcclient_login_form($texte,$contexte){
    ...
    AND $oidc = sql_getfetsel('oidc','spip_auteurs','login='.sql_quote($login))   // Le login SPIP est-il lié à un user_ID OIDC ?
    ) {
        $message = _T('oidcclient:form_login_oidcclient_ok') . $oidc
