    ...
    if ( strpos($prompt, 'none') !== false ) {
        ...
        */
        ...
        if ( strpos($prompt, 'login') !== false ) {  
            ...
            exit(
                // Display login form
                include __DIR__ . '/identification/' . LOGIN_FORM . '/login.php' 
            );
        ...
        if ( $prompt == 'consent' OR $continue_with_consent == true ) {  //[dnc24]
            ...
            if ( $count = count($scopes_to_grant) ) {
                ...
                exit(     
                    include OIDCSRV_ROOT_PATH . 'identification/grant/grant.php' 
                );
...
if ( ! empty($password) OR 'login' == $return_from ) {
    ...
    if ( $is_authorized AND $login_with_tfa ) {
        ...
        exit(
            // Display login form
            include OIDCSRV_ROOT_PATH . 'identification/' . TFA_PROVIDER . '/login.php'
        );    
