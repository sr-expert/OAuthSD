...
if ( !empty($thetexte1) ) {   //[dnc16]
    ...
    $texte1 = 'Identifiez-vous sur ' . $theclient . ' de ' . $thename;
}

if ( !empty($thetexte2) ) {   //[dnc16]
    $texte2 = $thetexte2;
} else {
    $texte2 = 'Pour  votre protection, '  . $thename . ' utilise <a href="http://oa.dnc.global">OAuth Server by DnC</a> : les mots de passe ne peuvent être interceptés au moment de leur saisie, ne circulent pas sur Internet, ne sont enregistrés nulle part !';
}
...
echo(' 
    ...
    <br />
    <div class="nologin">
    Vous n\'avez pas d\'identifiants ? <a href="/?page=editer_user_ext&id_client=' . $id_client . '&client=' . $theclient . '&nom=' . $thename . '&pswdl=' . PSWD_LENGTH . '">Inscrivez-vous ici</a>.     
    </div>
